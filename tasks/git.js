...
function rebase(git, branch, remote, dir) {
  return spawn(git, ['pull', '--rebase', remote, branch], dir);
}
...
module.exports = function(grunt) {
  ...
  grunt.registerTask('publish', 'Publish (commit, push) with git.',
    ...
    var remote = 'origin';
    grunt.log.writeln('Adding ' + files + ' to ' + branch);
    add(options.git, files, options.dir).
        then(function() {
          grunt.log.writeln('Committing to ' + branch);
          return commit(options.git, options.message, options.dir);
        }).
        then(function() {
          grunt.log.writeln('Rebasing ' + branch);
          return rebase(options.git, branch, remote, options.dir);
        }).
        then(function() {
          grunt.log.writeln('Pushing ' + branch);
          return push(options.git, branch, remote, options.dir);
        }).
        then(function() {
          done();
        }, function(error) {
