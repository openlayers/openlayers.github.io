...
function ramp(min, max, colors) {
	const stops = [];
	for (let i = 0; i < colors.length; i++) stops.push(min + i / (colors.length - 1) * (max - min), colors[i]);
	return { color: [
		"interpolate",
		["linear"],
		["band", 1],
...
var FTW_URL = "https://data.source.coop/ftw/global-data/predictions/zarr/alpha/global.zarr";
var datasets = {
	"ftw-rgb": {
		source: {
			url: FTW_URL,
...
function render() {
	...
	map = new Map({
		...
		target: "map",
		view: getView(source, withLowerResolutions(2), withHigherResolutions(2), withExtentCenter())
	});
}
