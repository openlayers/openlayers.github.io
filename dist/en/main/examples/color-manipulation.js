...
function rgb2hcl(pixel) {
	...
	const x = xyz2lab((.4124564 * red + .3575761 * green + .1804375 * blue) / Xn);
	const y = xyz2lab((.2126729 * red + .7151522 * green + .072175 * blue) / Yn);
	const z = xyz2lab((.0193339 * red + .119192 * green + .9503041 * blue) / Zn);
	const l = 116 * y - 16;
	const a = 500 * (x - y);
	const b = 200 * (y - z);
...
function hcl2rgb(pixel) {
	...
	x = Xn * lab2xyz(x);
	z = Zn * lab2xyz(z);
	pixel[0] = xyz2rgb(3.2404542 * x - 1.5371385 * y - .4985314 * z);
	pixel[1] = xyz2rgb(-.969266 * x + 1.8760108 * y + .041556 * z);
	pixel[2] = xyz2rgb(.0556434 * x - .2040259 * y + 1.0572252 * z);
	return pixel;
}
function xyz2lab(t) {
...
var raster = new RasterSource({
	sources: [new StadiaMaps({ layer: "stamen_watercolor" })],
